...
async function buildContext(userInput, options = {}) {
  ...
  const system = [
    ...
    '【选曲规则】① play 数组包含 8-12 首 ② 所有歌曲必须来自上方"可选曲库"，歌名和艺人名保持原样，不得修改 ③ 禁止推荐"用户明确不喜欢的歌"、"近期已播放"或"当前队列中已有的歌"里的任何一首 ④ 如果用户点过喜欢，优先延续这些歌或这些艺人的气质、编曲、情绪线索，但不要机械重复同一首 ⑤ 根据当前时间和用户品味从曲库中挑选最契合的几首 ⑥ 如果用户只是闲聊不涉及音乐，play 数组可为空 ⑦ artist 字段必须填写原唱艺人全名，禁止填写翻唱歌手、配乐版、钢琴版、纯音乐版等非原唱版本的艺人名 ⑧ replace_pool 是布尔值：只有当用户明确要求完全换一种风格、情绪或方向，导致当前池子整体都不该继续时，才返回 true；普通的补充、微调、延续、陪聊后一两句点歌都返回 false',
